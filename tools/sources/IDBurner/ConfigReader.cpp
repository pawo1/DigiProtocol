...
std::map<std::string, std::map<std::string, std::string>> ConfigReader::dumpConfiguration() {
    return configuration;
}

int ConfigReader::updateConfig() {
    file.open( file_path, std::ios::in);
    ...
    while(std::getline(file, line)) {
        ...
        if(temp_line.size() >= 1) {
            if(section == key_section && temp_line[0] == key) {
                ...
                configuration[key_section][key] = new_value;
                keyfound = true;
            } else
...
int ConfigReader::addRecord(std::string key_section, std::string key, std::string value) {
    ...
    configuration[ key_section ][ key ] = value;
...
int ConfigReader::deleteRecord(std::string key_section, std::string key) {
    ...
    configuration[key_section].erase(key);
...
bool ConfigReader::existsValue(std::string key, std::string value) {
    int counter = 0;

    for(auto const &element1 : configuration) {
        std::string temp = configuration[element1.first][key];
...
std::string ConfigReader::getSectionByValue(std::string key, std::string value) {
    for(auto const &element1 : configuration) {
        std::string temp = configuration[element1.first][key];
        if(temp == value) return element1.first;
    }
    return "";
}

int ConfigReader::renameSection(std::string old_name, std::string new_name) {
    ...
    configuration[new_name] = configuration[old_name];
    configuration.erase(old_name);

    return result;
}

int ConfigReader::deleteSection(std::string del_section) {
    ...
    while(std::getline(file, line)) {
        ...
        updateSection(removeSpaces(line), section);

        if(section != del_section)
            temp_data.push_back(line+"\n");
    ...
    configuration.erase( del_section );

    return result;
}

bool ConfigReader::existSection(std::string section) {
    if(configuration.count(section)) return true;
    return false;
}
